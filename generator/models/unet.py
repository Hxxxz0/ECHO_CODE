...
class EchoUnet(nn.Module):
    """1D convolutional UNet with cross-attention for text-to-motion generation."""
    def __init__(self,         
                 input_feats,
                 base_dim = 128,
                 dim_mults=(1, 2, 2, 2),
        ...
        print(f'CFG: masking text prompt with {self.cond_mask_prob} probability during training')
    ...
    def forward(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: additional arguments (e.g., lengths) for compatibility
...
if __name__ == "__main__":
    ...
    model = EchoUnet(
        ...
        base_dim=base_dim,
        dim_mults=dim_mults,
        adagn=True, zero=True,
        dropout=0.1, no_eff=False,
        cond_mask_prob=0.1
    ).to(device)
    bs = 1
    x = torch.rand((bs, num_frames, n_feats), dtype=torch.float32).to(device)
    timesteps = torch.randint(low=0, high=1000, size=(bs,)).to(device)
    y = ['a person walks forward']
