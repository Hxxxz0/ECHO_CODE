...
class DDPMTrainer(object):
    ...
    def __init__(self, args, model,accelerator, model_ema=None):
        ...
        # Enable rotation smoothness loss only for Transformer models
        self.use_rotation_smooth_loss = hasattr(args, 'model_type') and args.model_type == 'transformer'
        if self.use_rotation_smooth_loss:
            accelerator.print('[INFO] Rotation smoothness loss enabled for Transformer model')
        ...
        if self.accelerator.is_main_process:
            ...
            self.writer = SummaryWriter(log_dir=pjoin(args.save_root,'logs_')+starttime[:16],comment=starttime[:16],flush_secs=60)  # Named by experiment timestamp
        self.accelerator.wait_for_everyone()

        # For Transformer models, use get_optim_groups to properly separate
        # decay/no_decay parameters (biases, LayerNorm, positional embeddings should not decay)
        model_type = getattr(args, 'model_type', 'unet')
        if model_type == 'transformer' and hasattr(self.model, 'get_optim_groups'):
            optim_groups = self.model.get_optim_groups(weight_decay=self.opt.weight_decay)
            self.optimizer = optim.AdamW(optim_groups, lr=self.opt.lr)
            accelerator.print('[INFO] Transformer: using get_optim_groups() for proper weight decay separation')
        else:
    ...
    def forward(self, batch_data):
        ...
        # Pass lengths to enable key_padding_mask for variable-length sequences
        self.prediction = self.model(x_t, t, text=caption, lengths=cur_len)
    ...
    def masked_l2(self, a, b, mask, weights):
        """Compute masked L2 loss for motion reconstruction.
        
        Args:
            a: prediction (B, T, D)
            b: target (B, T, D)
            mask: validity mask (B, T)
            weights: timestep weights (B,)
        """
        ...
        loss_rec = (loss * weights).mean()

        return loss_rec
    
    def rotation_smoothness_loss(self, pred, target, mask, weights):
        """Compute rotation smoothness loss for root_rot_6d (Transformer only).
        
        This loss penalizes large jumps in root rotation between consecutive frames,
        addressing the discontinuity issue observed in Transformer outputs.
        
        Args:
            pred: prediction (B, T, D) - full 38D features
            target: target (B, T, D) - not used, kept for consistency
            mask: validity mask (B, T)
            weights: timestep weights (B,)
        ...
        """
        # Extract root_rot_6d (last 6 dimensions of 38D)
        pred_rot = pred[..., 32:38]  # (B, T, 6)
        
        # Compute frame-to-frame differences (velocity)
        diff = pred_rot[:, 1:] - pred_rot[:, :-1]  # (B, T-1, 6)
        
        # Mask for valid transitions (both frames must be valid)
        mask_diff = mask[:, 1:] * mask[:, :-1]  # (B, T-1)
        
        # Use Huber loss to be robust against large jumps while still penalizing them
        # delta=0.1: transitions larger than 0.1 are penalized linearly (not quadratically)
        loss_smooth = torch.nn.functional.huber_loss(
            diff, torch.zeros_like(diff), reduction='none', delta=0.1
        ).mean(dim=-1)  # (B, T-1)
        
        # Apply mask and weights
        loss_smooth = (loss_smooth * mask_diff).sum(-1) / (mask_diff.sum(-1) + 1e-8)  # (B,)
        loss_smooth = (loss_smooth * weights).mean()  # scalar
        
        return loss_smooth

    def backward_G(self):
        loss_logs = OrderedDict({})
        mse_loss_weights = torch.ones_like(self.timesteps).float()
    ...
    def load(self, model_dir):
        ...
        # Load model weights first (required)
        self.model.load_state_dict(checkpoint['encoder'], strict=True)
        if self.model_ema:
            self.model_ema.module.load_state_dict(checkpoint["model_ema"], strict=True)
        # Load optimizer; skip if param_groups mismatch (e.g., architecture/optimizer config changed)
        try:
            self.optimizer.load_state_dict(checkpoint['opt_encoder'])
        except (ValueError, KeyError) as e:
            self.accelerator.print(
                f'[WARN] Skip loading optimizer state ({e}); continuing with fresh optimizer.'
            )
    ...
    def train(self, train_loader):
        ...
        if self.opt.is_continue:
            # Load from original experiment dir: use continue_model_dir if provided, else current model_dir
            load_dir = getattr(self.opt, 'continue_model_dir', None) or self.opt.model_dir
            if os.path.isabs(self.opt.continue_ckpt):
                model_path = self.opt.continue_ckpt
            else:
                model_path = pjoin(load_dir, self.opt.continue_ckpt)
            it = self.load(model_path)
            # When fine-tuning across datasets, checkpoint iteration may exceed new num_train_steps;
            # reset to 0 in that case to reuse pretrained weights only
            if it >= self.opt.num_train_steps:
                self.accelerator.print(f'[Finetune] checkpoint it={it} >= num_train_steps={self.opt.num_train_steps}, reset it=0')
                it = 0
            else:
        ...
        for epoch in range(0, num_epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                if it % self.opt.save_interval == 0 and self.accelerator.is_main_process: # 500
                    ...
                    self.save(pjoin(self.opt.model_dir, f'ckpt_{it}.tar'), it)
                self.accelerator.wait_for_everyone()
